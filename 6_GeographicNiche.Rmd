...
G Evelyn Hutchinson proposed that _the niche is an n-dimensional hypervolume..._ - [**Hutchinson 1957**](https://doi.org/10.1101%2Fsqb.1957.022.01.039)
...
The "prediction" is usually based on a statistical model that uses the known localities to estimate the probability of a species occurring across the given (n-dimensional) environmental space.
]
...
.footnote[Conceptual figure by [**Ragnvald**](https://commons.wikimedia.org/wiki/File:Predicting_habitats.png)]
]
---
...
Projected species distributions are used in many ways, including in conservation. Our National Screening Tool for environmental impact authorizations for developments includes modelled distributions of animals. Sites where sensitive species are predicted to occur are required to do more thorough site inspection studies.
...
We can build SDMs based on models that estimate population growth across different environmental conditions based on their **demographic parameters** (typically related to _birth, death and dispersal_ rates).
...
Population size/stability are determined by ***key demographic rates*** representing birth and death:
...
Together $^1$ , the covariation between these parameters and environmental variables can be used to determine where the population growth rate ($r$) of *Protea punctata* allows the species to occur.

.footnote[
$^1$ Note that this species is killed by fire, so there is no adult survival to consider. Similarly, dispersal rates are so low/slow at the spatial scale of the model that they can be ignored.]
]
---
...
Focuses on the organism's role or **function** in the ecosystem, often in relation to trophic position (consumers, predators, etc) other interactions like competition, mutualisms and **resource use**.

These are tricky things to try to include! You'd need to know the occurrence or abundance of all other species' your focal species interacts with across its range! 

...and how those interactions change across environmental gradients! 
...and how they affect the species' demographic parameters across its range!
...
>*One of the most common applications of the concept of the niche is Species Distribution Modelling (also known as Ecological Niche Modelling), which typically invoke the Grinnellian niche and Hutchinson's n-dimensional hypervolume concept.*
