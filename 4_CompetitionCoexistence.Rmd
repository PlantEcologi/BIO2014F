...
The **competitive exclusion principle** is often attributed to Gause, and sometimes called Gause's law, but there were earlier observations by Darwin, Grinnell and others, e.g.
...
- if $= 1$ then the effect of species 2 on species 1 is the same as the effect of species 1 on species 1
  - _the equation would essentially reduce to the logistic growth curve and you could treat all individuals as one species_
- if $> 1$ species 2 has a greater effect on species 1
- if $< 1$ species 2 has a smaller effect on species 1
- if $= 0$ there is no effect of species 2 on species 1   - _i.e. the presence of individuals of species 2 is irrelevant for species 1 and the new term effectively disappears (= 0) and the equation would essentially reduce to the logistic growth curve for species 1_
]
...
- Notice that the size of $N_2$ at this point depends on $\alpha_{12}$. Population growth declines to 0 (the ZNGI) at lower $N_2$ when $\alpha_{12}$ is large (species 2 has a stronger competitive effect).
